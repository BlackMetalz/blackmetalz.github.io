...
There are some hooks type: `PreToolUse` (Hooks that run before a tool is called), `PostToolUse` (Hooks that after before a tool is called)

Defination, same for skill if can be global, project or project but not gonna commit.

![Hooks Definitions](./images/2026/04/04.png)
...
So it will format (go fmt) and do static analysis tool (go vet). After any of tool in matcher list (Write|Edit|MultiEdit) was used. It can be Claude fix bug before telling us it is finished.
