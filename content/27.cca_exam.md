...
│                      Linux Kernel                           │
│  ┌─────────┐    ┌─────────┐    ┌─────────┐    ┌─────────┐  │
│  │   XDP   │───►│   TC    │───►│ Socket  │───►│ cgroups │  │
│  │ (ingress)│    │(ingress/│    │  Ops    │    │  (pod)  │  │
│  │         │    │ egress) │    │         │    │         │  │
│  └─────────┘    └─────────┘    └─────────┘    └─────────┘  │
│       │              │              │              │        │
│       └──────────────┴──────────────┴──────────────┘        │
...
│                    iptables Flow                            │
│                                                             │
│   Packet ──► Rule 1 ──► Rule 2 ──► Rule 3 ──► ... ──► Rule N│
│              (match?)   (match?)   (match?)       (match?)  │
│                                                             │
│   Problems:                                                 │
│   - O(n) complexity — linear rule matching                  │
│   - Performance degrades with more rules                    │
│   - No visibility into packet flow                          │
│   - Rules are IP-based, not identity-based                  │
└─────────────────────────────────────────────────────────────┘
```
...
│                      eBPF Flow                              │
│                                                             │
│   Packet ──► eBPF Program ──► Decision (O(1) hash lookup)   │
│                   │                                         │
│                   ▼                                         │
│              eBPF Maps                                      │
│              (hash tables)                                  │
...
│  │  │                 │         │                         │    │   │
│  │  │ - CRD management│         │ - Aggregates flows      │    │   │
│  │  │ - IPAM          │         │ - Serves Hubble UI/CLI  │    │   │
│  │  │ - Cluster Mesh  │         │                         │    │   │
│  │  └─────────────────┘         └─────────────────────────┘    │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
...
│          ▼                                   ▼                   │
│   ┌─────────────┐                     ┌─────────────┐           │
│   │   Cilium    │     VXLAN/Geneve    │   Cilium    │           │
│   │   Agent     │◄═══════════════════►│   Agent     │           │
│   │ 192.168.1.1 │      Tunnel         │ 192.168.1.2 │           │
│   └─────────────┘                     └─────────────┘           │
│                                                                  │
...
|------|-------------|----------|--------------|
| `tunnel=vxlan` | VXLAN | Default, works anywhere | None |
...
│                                                             │
│   Pod A                                                     │
│   Labels: app=frontend    ──────► Cilium ──────► Pod B     │
│   Identity: 12345                  Rule:        Labels:     │
│                              "Allow identity              app=backend │
│                               with app=frontend"          Identity: 67890│
│                                                             │
│   Benefits:                                                 │
