...
- 1. Statement-based replication: master write exactly query we did, then slave replay it. Cons: will be issue if related to function like `NOW()`. Because `NOW()` from master at 12:00 will be something like 12:03 in slave, diff issue bro!
- 2. Row-based replication: instead of write query SQL, master will record every change of "rows" (data changed). So Slave will replay exactly what is binlog, no diff issue. Cons: if we run update that target 1M rows, Binlog will increase way too big instead write simple 1 sql query.
- 3. Mixed-base replication: This is default method of modern MySQL and is exactly what AWS RDS use in common I guess xD. It will make priority for Statement-based, but if any query contain something dangerous like NOW() , UUID(), RAND(), it will convert to Row-based to make data consistency!
...
- 1. IO Thread (Ship): This shipper have a mission that connect to Master, if see any new Binlog, it will copy back into Slave node to a temp file called "Relay Log".
- 2. SQL Thread (Worker): This worker just collect row/qery from Relay Log to replay data into Slave node.
...
- Interface Endpoint: it creates 1 ENI (Elastic Network Interface) with private IP that attach into your `subnet`, instance in your VPC route to that subnet can use ENI, support most AWS services. Then your application can use that private domain name (if enable Private DNS) to connect to AWS Services. Cost money by hour and GB data processed!
