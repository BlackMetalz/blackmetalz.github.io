...
ArgoCD is a workload that commonly runs in the **argocd** namespace. I thought ArgoCD's watch mechanism was the same as K8s's watch mechanism, but it turns out there's a little difference, let's figure it out!

### Difference between ArgoCD and Kubernetes

| Property | K8s Controller Manager | ArgoCD (Application Controller) |
| -- | -- | -- |
| Source of desired state | etcd | Git repository (outside of cluster) |
...
- **argocd-server**: API Gateway, takes requests from UI/CLI, checks auth/RBAC of user only.
- **argocd-repo-server**: clones Git, runs **helm template** or **kustomize build** to render raw yaml.
- **argocd-application-controller**: where we run reconciliation loop to see the drift between the desired state (rendered YAML from repo-server) versus live state in k8s. This is the only component that calls the API to apply manifests.
- **argocd-redis**: Caches the rendered manifests. Without Redis, each sync check, repo-server will need to clone Git and render helm again! That is why we need fuckin' cache!

### Dynamic View

Here is the flow (combining [Architecture](https://argo-cd.readthedocs.io/en/stable/operator-manual/architecture/) and [Webhook](https://argo-cd.readthedocs.io/en/stable/operator-manual/webhook/) docs):

![ArgoCD Flows](./images/2026/07/18.png)

### Kubernetes Controller Manager: Real-time Watch (Event-Driven)

In K8s, Controllers (like the ReplicaSet Controller, Deployment Controller) set up an [HTTP Streaming Watch](https://kubernetes.io/docs/reference/using-api/api-concepts/#efficient-detection-of-changes) mechanism to the kube-apiserver:
...
- **First loop** (ArgoCD): compare Git (desired) versus K8s object spec read via kube-apiserver (this is `actual` from ArgoCD's view, but it's also exactly what feeds into loop 2 below as `desired`). If the GitOps repo changes, or someone drifts the live spec away from Git (and selfHeal: true is set), ArgoCD re-applies via kube-apiserver to overwrite the drift back to match Git.
...
And one more thing worth mentioning: [Kube-operators](https://kubernetes.io/docs/concepts/extend-kubernetes/operator/) use the same mechanism as the **second loop**
...
- ArgoCD loop: reads Git, sees **replicas:2**, apply to K8S.
- K8S HPA Controller Loop: sees pod resource reached condition to scale, updates Deployment spec in K8S cluster to **replicas:5** for example.
- ArgoCD loop: (if enable **selfHeal: true**): Scan cluster, see actual **replicas:5** while Git defined **replicas:2** (Drift). ArgoCD updates the deployment spec to match what Git defined.
- K8S HPA Controller Loop: scale up again because pod reached condition to scale.
- Result: It creates an infinite loop causing unnecessary churn (constant rollouts, wasted resources) and potential service disruption.
...
spec:
  ...
  destination:
    server: 'https://kubernetes.default.svc' # Cluster API URL (this refers to the local cluster). We could point to another cluster if we have one.
...
- **argocd-repo-server** will checkout git, list the folders/files that match the pattern, then return a raw parameter list to **applicationset-controller**. After that, **applicationset-controller** renders the templates with those parameters.
...
After that, we have our first connected repo:
...
Oh, what went wrong to prevent ArgoCD from syncing the application?
...
I'm pretty fuckin' sure that's caused by the destination we defined here that allows only namespaces with this pattern: **lab-***
...
They will appear in **bootstrap-app**

![bootstrap-app](./images/2026/07/12.png)

Ok, we have set up a connected repo, we have set up an **applicationSet** that generates ArgoCD Applications. Next, we will move to cluster registration. We commonly register clusters via **argocd-cli**, but I want to show you another approach that uses the GitOps way!
...
We create a raw cluster secret locally named: **raw-cluster-register.yaml**. Please notice that **bearerToken** is actually field **token** under users section. If you are using managed k8s for testing like me, remove **tlsClientConfig**.
...
Then run the fuckin command to encrypt it and store it in the bootstrap repo.

```bash
kubeseal --format=yaml < raw-cluster-register.yaml > bootstrap/cluster-registration.yaml
```

Commit it to the **gitops-bootstrap** repo. Then refresh **bootstrap-app**, and we will have a new SealedSecret resource
...
![git to matrix generator](./images/2026/07/19.png)

**Why switch from Git generator to Matrix generator?** The original Git generator only scans folders in `apps/*` and generates one Application per folder, but the `destination.server` in the template is hardcoded to a single cluster. It has no awareness of other registered clusters. By switching to a Matrix generator (`clusters: {}` × `git: {directories: apps/*}`), we create a **cross-product**: every folder × every registered cluster = one Application. So 4 folders × 2 clusters = 8 Applications instead of just 4. And when we add Cluster 3 in the future, `clusters: {}` auto-discovers it, no ApplicationSet changes needed!
...
![fix AppProject](./images/2026/07/17.png)
...
- Disadvantage: Single Point of Failure. If Cluster 1 goes down, we cannot deploy or update workloads on Clusters 2 and 3. However, existing workloads will continue running fine — this only affects new deployments.
