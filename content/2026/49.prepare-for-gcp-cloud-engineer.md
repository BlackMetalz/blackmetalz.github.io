...
- Cloud Spanner: like DynamoDB in AWS (able scale horizontally to handle write loads that exceed the capacity of a single server)
...
- **Workforce identity pools** are Google Cloud organization-level containers used in **Workforce Identity Federation** to manage external user identities (like employees or partners).
