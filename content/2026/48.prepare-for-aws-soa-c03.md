Title: Prepare for AWS Certified CloudOps Engineer Associate SOA-C03 2026 exam
Date: 2026-07-20
...
- Supports EC2 instances and RDS instances. (Do NOT use this for EC2 ASG, because if it stops an instance in ASG, ASG thinks that instance is dead/unhealthy then terminates and recreates it. For ASG scheduling, use ASG Scheduled Actions/Scaling instead!)
...
Holy fuck, remember AMI is region locked. You can copy it to other regions, but cannot share it directly. Sharing with other AWS accounts is region-wide (available across all AZs in that region, not locked to single AZ).
...
Operates at Layer 3 - Ip Packets. Exam perspective: Use GENEVE protocol on port 6081 (remember 6081, not 6801 or other shit!)
...
It is fuckin' **managed NFS**. Available at region level, it means you can mount NFS from any EC2 in any AZs in that fucking region. We have lifecycle management feature - move file after N days, as a related storage service in AWS, it will have standard storage tiers:
...
Use case: let user download file from private S3 bucket without create IAM user or make public bucket (Like SAS from Azure Storage!)
...
Don't mistake S3 Versioning propagation with S3 Replication Time Control (RTC) which has a 15-minute SLA. Versioning is active immediately once enabled, so no fucking propagation delay of 15 minutes here! If you get a 404 NoSuchKey, it's just because the key doesn't exist, not because versioning is lagging.
...
| **Archive** | **S3 Glacier Flexible Retrieval** | Archive (Rarely accessed data) | 90 days | 1 minute to 12 hours |
...
- Available within AZ, Cross AZ or Cross Region. Replication is ASYNC.
...
So for example, we can restrict specific AWS account in our Org that not able list s3 buckets by use Deny action on s3:ListAllMyBuckets, even that account has Admin or an IAM user has policy that allow s3:ListAllMyBuckets, why?

- **Explicit Deny wins**: In AWS IAM policy evaluation, an explicit deny always overrides any allow, no matter where it is defined (in IAM policies, Resource policies, or SCPs).
- **SCPs set the limit**: Service Control Policies (SCPs) define the maximum available permissions for the accounts in an organization. If an SCP denies an action (like s3:ListAllMyBuckets), no user or role in that account can perform it, even if their local IAM policies explicitly allow it.
...
- A **Public Hosted Zone** is a container that holds DNS records for a domain that you want to be accessible on the public internet.
- A **Private Hosted Zone** is a container that holds DNS records for a domain that you want to resolve only within one or more of your Amazon VPCs.
- Both **Public/Private Hosted Zone** available inside Amazon **Route 53**. Nothing to say more about Public Hosted Zone but for Private Hosted Zone, you will be able to select the VPCs you want to associate it with!
...
Pretty sure it is fuckin' easier than NAT Instance, because it is fuckin' managed service. You only need to add route from your instance to NAT Gateway, that's all! But we still need to remember litte things:

- Public NAT Gateway (Standard Internet Access) -> Must be in a Public Subnet.
- Private NAT Gateway (Internal Routing Only) -> Can be in a Private Subnet

Conclusion:

- Need to download packages from the internet? Public subnet only.
- Need to connect to an overlapping private VPC/on-premises network without internet access? Private subnet.
...
I used this course to practice and test my knowledge: 

[Prepare for AWS Certified CloudOps Engineer - Associate exam](https://www.udemy.com/course/prepare-for-aws-certified-cloudops-engineer-associate-exam/)

---

# Conclusion

- Not too bad, but yeah, this article is lack of domain 4 and 5 I guess or I didn't remember them well..
![report](./images/2026/07/07_00.png)

- I didn't remember correct what **GWLB** do exactly during exam or not sure **Amazon Macie** can do scan sensitive data on S3 only or able to scan on other datasource.
- The amount of service you need to memorize is a lot.
- It still easier a lot than Azure AZ-104 LOL. Possible done within a week if you worked with AWS before (I haven't work in AWS single day, just doing some labs....).
