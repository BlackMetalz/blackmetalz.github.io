...
If you are short on time, read everything unmarked, do the [Exam Checklist](#exam-checklist), skim the [Quick Recall cheat sheet](#quick-recall-exam-day-cheat-sheet), and ignore the rest.
...
- **Data (Major)** — *self-supervised* learning (Phase 1.5) removed the human-labeling
  bottleneck, so the raw internet became training data.
- **Compute (Major)** — Tensor Cores plus ever-lower precision (FP32 → BF16 → FP8 → FP4):
...
1. **Data, reach here first.** More real data, or **augmentation** (crop, rotate, paraphrase — synthetic variety from what you already have). Nothing beats showing the model more of the world.
2. **Architecture.** Fewer layers or parameters, because a smaller model has less room to memorize. **Dropout** randomly disables neurons during training so they can't lean on each other.
3. **Regularization & optimization.** **Weight decay / L1 / L2** penalize oversized weights. **Early stopping** ends the run when validation loss turns upward, instead of guessing an epoch count.
...
- **DPU ([BlueField](https://www.nvidia.com/en-us/networking/products/data-processing-unit/)) — infrastructure offload.** Takes networking, storage and security work off the host CPU so the CPU can stay busy feeding the GPUs. Programmed with the **DOCA(Data Center on a Chip Architecture)** SDK. If we need one thing for reflection, we can think about AWS Nitro I believe!
...
| | What it is | Analogy |
|---|---|---|
| **[TensorRT](https://developer.nvidia.com/tensorrt)** | Compiler/optimizer that makes one model run fast (layer fusion, kernel tuning, quantization) | Compiling your code with |
...
- **[NGC](https://catalog.ngc.nvidia.com/)** — NVIDIA's registry of GPU-optimized containers, pretrained models and Helm charts. Think like Docker Hub or container registry.
...
[Run:ai](https://www.nvidia.com/en-us/software/run-ai/) is worth understanding because it's essentially "Slurm's scheduling brain, but for Kubernetes" — teams get quotas, idle quota gets borrowed and reclaimed, and distributed jobs start all-or-nothing instead of deadlocking half-scheduled.

*(Note: Slurm uses "gang scheduling" to mean time-slicing oversubscribed jobs — see [`PreemptMode=GANG`](https://slurm.schedmd.com/gang_scheduling.html). Everyone else uses it to mean all-or-nothing co-allocation)*
