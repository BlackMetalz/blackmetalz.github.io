...
|---|---|---|
| Essential AI Knowledge | 38% | Phase 1 |
| AI Infrastructure | 40% | Phases 2-5, 8 |
...
| `Deployment` + HPA | Inference service ([Triton](https://developer.nvidia.com/triton-inference-server), [NIM](https://www.nvidia.com/en-us/ai/)) | Long-running, scales with request load |
...
Two of these land directly on infrastructure: **batch size drives GPU memory**, and **checkpointing drives storage bandwidth** — every GPU writes at the same instant and they all sit idle until the write completes. Checkpoint sizing is a real capacity input, not an afterthought.

### 1.4 How Models Learn
...
- **H200 = H100 compute + better memory** — 141GB HBM3e, ~1.4x memory bandwidth. Drop-in compatible with HGX H100 systems. **Best for large-model inference and long context**, and now you know why (Phase 1.6: decode is memory-bandwidth bound).
...
Detailed specs are in [Appendix A](#appendix-a-hardware-spec-reference) — reference material, not memorization material.
...
| **FP4 / INT8** | — | Inference quantization. Blackwell's NVFP4 |
...
PCIe Gen5 x16 gives ~64 GB/s per direction. NVLink 4 gives 900 GB/s — **roughly 7x**. Run AllReduce over PCIe and your expensive GPUs spend most of their life waiting.

- **[NVLink](https://www.nvidia.com/en-us/data-center/nvlink/)** — direct GPU-to-GPU interconnect bypassing PCIe entirely (H100: 18 links × 50 GB/s = 900 GB/s).
...
| Lossless | Native, credit-based flow control | Must be configured (PFC + ECN, together called DCQCN) | Built in |
...
Remember the checkpoint problem from Phase 1.3: hundreds of GPUs writing simultaneously, all idle until the write finishes. Checkpoint burst bandwidth is usually what sizes the filesystem, not average throughput.
...
[ Frameworks ]              PyTorch, JAX, TensorFlow  │  RAPIDS (data science)
            ▲
[ Accelerated Libraries ]   CUDA, cuDNN, NCCL, DOCA
            ▲
...
- **[NCCL](https://developer.nvidia.com/nccl)** — the collective communication library for multi-GPU and multi-node (`AllReduce`, `AllGather`, `Broadcast`, `ReduceScatter`), optimized for NVLink and InfiniBand. **Any question about optimizing inter-GPU communication is a NCCL question.**
...
The most useful skill here, and a common question format — you're given telemetry and asked what's wrong. **GPU utilization alone tells you nothing**; read it alongside everything else:
...
- **Power density** — a DGX H100 draws ~10.2 kW; a GB200 NVL72 rack draws ~120 kW. A traditional rack is 5-10 kW. You cannot drop these into an existing datacenter and hope.
- **Cooling** — air cooling runs out around 40-50 kW per rack. **Direct-to-chip liquid cooling** (cold plates) is required for GB200 NVL72, not optional.
- **PUE (Power Usage Effectiveness)** = total facility power ÷ IT equipment power. **Closer to 1.0 is better.** 1.5 means half your IT load again is spent on cooling and losses.
- **Also plan for**: redundant power (N+1, 2N), hot/cold aisle containment, floor weight, and cable runs for high-speed fabrics.
- **On-prem vs. cloud vs. hybrid** — on-prem wins on long-term cost *at high utilization* and on data control; cloud wins on elasticity and zero CapEx; hybrid keeps sensitive data local and bursts out. Utilization is the deciding number.
...
- Given utilization figures, name the bottleneck and the fix.
- Explain what an Xid error is and where it appears.
- Outline the MLOps lifecycle; distinguish data drift from concept drift.
- Explain PUE and why 120 kW racks need liquid cooling.
- Compare on-prem, cloud and hybrid on cost, control and elasticity.

---

# Appendix A: Hardware Spec Reference
