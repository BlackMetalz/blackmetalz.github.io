...
- [How Cilium CNI works in K8S](https://blackmetalz.github.io/container-networking-series-part-4-how-cilium-cni-works-in-k8s.html)
...
## 5. Simple flow for creating a pod

When we submit a request to create a pod to the API Server:

- Kubelet watch API Server via watch mechanism, see the fucking pod has been scheduled to Kubelet's node, then pull the pod spec to handle pod creation.
- Kubelet calls the CRI plugin (e.g., Containerd) to create a container. Note: CRI (Container Runtime Interface) is the interface, Containerd is one implementation of it.
- Containerd finishes creating the container and calls the CNI binary for network setup.
- The runtime passes config (JSON) via stdin + network info via env vars.
- CNI binary processes the config, sets up networking, and returns the result via stdout (JSON).
