...
If the CNI Plugin (like `bridge`) handles "plugging the wires," then **IPAM** handles the "ledgering."

* It manages IP ranges (Subnets), tracking which IPs are assigned and which are free.
* In this lab, we use `host-local`. It stores a simple IP database directly on the Node's disk (in `/var/lib/cni/networks/`).
...
```json
{
    ...
    "interfaces": [
        {
            "name": "veth6d427827",
            "mac": "f2:c0:f7:20:1f:90"
        },
        {
            "name": "eth0",
            "mac": "66:cd:18:04:95:6a",
...
**The Pod lives!** Kubelet detects the new config file → retries the Pod Sandbox creation → this time containerd calls `/opt/cni/bin/ptp` (exactly as we did in Step 3) → Network setup succeeds → Pod transitions to `Running`.
