...
```
Client Request
├─ Ingress Gateway (edge traffic)
   ├─ VirtualService (routing rules)
   │  └─ DestinationRule (load balancing, TLS settings)
...
└──────┬──────┘     └──────┬──────┘     └──────┬──────┘
       ...
       └───────────┬───────┴───────────────────┘
                   ▼
          ┌───────────────┐
          │   ztunnel     │  ◄── L4 processing (mTLS, basic routing)
          │  (per node)   │
          └───────┬───────┘
                  ▼
          ┌───────────────┐
          │   Waypoint    │  ◄── L7 processing (optional, per namespace/service)
          │    Proxy      │
          └───────────────┘
```
...
| L4 mTLS | ✅ | ✅ (ztunnel) |
| L7 features | ✅ (always) | ✅ (needs waypoint) |
...
│  (entry point)│      │ (WHERE to route)│      │ (HOW to route)   │
└──────────────┘      └─────────────────┘      └──────────────────┘
                              │                         │
                              ▼                         ▼
...
Mesh-wide (MeshPolicy)
    └─► Namespace-level
          └─► Workload-level (most specific wins)
```
...
│                                                                     │
│   Gateway ──────► VirtualService ──────► DestinationRule            │
│   (entry point)   (routing rules)        (traffic policy)           │
│                         │                       │                   │
│                         ▼                       ▼                   │
│                   - host matching         - subsets                 │
│                   - URI matching          - load balancing          │
│                   - header matching       - circuit breaker         │
│                   - traffic split         - connection pool         │
│                   - fault injection       - outlier detection       │
│                                                                     │
│   ServiceEntry ──────► External Services                            │
│   WorkloadEntry ─────► VMs / Non-K8s Workloads                      │
│                                                                     │
├─────────────────────────────────────────────────────────────────────┤
│                           SECURITY                                  │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│   PeerAuthentication ──────► mTLS Settings (server-side)            │
│   (STRICT/PERMISSIVE)        - mesh-wide                            │
│                              - namespace                            │
│                              - workload                             │
│                                                                     │
│   RequestAuthentication ───► JWT Validation                         │
│   (issuer, jwksUri)          - token validation only                │
│                                                                     │
│   AuthorizationPolicy ─────► Access Control                         │
│   (ALLOW/DENY/CUSTOM)        - source (who)                         │
│                              - operation (what)                     │
│                              - condition (when)                     │
│                                                                     │
├─────────────────────────────────────────────────────────────────────┤
...
│   istioctl proxy-status ────► Sync status with control plane        │
│   istioctl proxy-config ────► Envoy configuration                   │
│   istioctl dashboard ───────► Observability tools                   │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
