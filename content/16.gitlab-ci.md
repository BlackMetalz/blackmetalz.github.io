...
### Target Audience:
- DevOps engineers who are familiar with Jenkins and want to explore GitLab CI
- Developers who want to understand CI/CD pipeline basics with GitLab
- Teams considering migration from Jenkins to GitLab CI
- Intermediate level users who already know basic Git concepts

All lab repo will come from my public GitLab repo, so I can see them again in future xD, and you guy can understand what is going on.
...
--- 
# Basic example with GitLab CI (Linting + Testing + Build + SAST)
- I used Copilot to generate this example. Here is the link to public repo: [https://gitlab.com/kienlt-gitlab-ci-demo/sample-python-project](https://gitlab.com/kienlt-gitlab-ci-demo/sample-python-project)
...
- And here is the link to the pipeline in case of you are new to GitLab CI: [kienlt-gitlab-ci-demo/sample-python-project](https://gitlab.com/kienlt-gitlab-ci-demo/sample-python-project/-/pipelines/2147473044)
...
- Here is example when it failed with HIGH vulnerabilities: [gitlab.com/kienlt-gitlab-ci-demo/sample-python-project](https://gitlab.com/kienlt-gitlab-ci-demo/sample-python-project/-/jobs/12020989478). Lets me show you a screenshot for quick view
...
### SonarQube
I only want to introduce the pipeline script that run SonarQube in runner that is running under proxy and able to get Quality Gateway after pushed, it doesn't include how many issue with level Critical, High and Medium yet.
...
- **base64(username:password)** = **echo -n "username:password" | base64**
- Before minify:
```yaml
{
    "auths": {
...
- to this, use this website [https://jsonformatter.org/json-minify](https://jsonformatter.org/json-minify)
- After minify: 
```json
{"auths":{"docker.io":{"auth":"base64(username:password)"}}}
```

### Another template example that i'm using
- Link to repo: [https://gitlab.com/kienlt-gitlab-ci-demo/demo-real-template](https://gitlab.com/kienlt-gitlab-ci-demo/demo-real-template)
- Example for clean GitLab pipline configuration with template repo:
```yml
include:
  - project: 'kientl-demo-group/demo-real-template'
    ref: v0.0.7
    file:
      - '/templates/jobs/sonarqube.yml'
      - '/templates/jobs/build-and-scan-image.yml'
...
#=== SONARQUBE STAGE ===#
#=== 5.0.1 compatible with Sonarqube Server 10.5.xx ===#
sonarqube: # ← This is the KEY that links template and consumer
  variables:
    SONAR_PROJECT_KEY: "demo-project-key"
    SONAR_SOURCES: "." # scan all files in repo

# Add workflow to build stage
build-image: # ← This is the KEY that links template and consumer
  rules:
    - if: >
        $CI_PIPELINE_SOURCE == "merge_request_event" &&
        $CI_COMMIT_BRANCH != $CI_DEFAULT_BRANCH &&
        $CI_COMMIT_TAG == null
    - if: $CI_COMMIT_BRANCH == "kienlt-demo-branch"
    - if: $CI_COMMIT_TAG
```

### Build App for Window in GitLab CI that not using Window runner
...
GitLab pipeline config, this should give you overview how it be done!
...
build-windows:
  ...
  dependencies:
    - build-windows-libs
  timeout: 10m  # Maximum 10 minutes for entire job  
  variables:
  ...
  script:
    - |
      ...
      # Kill ALL Wine processes before exit to prevent hanging
      echo "Killing all Wine and background processes..."
      killall -9 wine 2>/dev/null || true
      ...
      echo "Process cleanup complete. Exiting..."
  after_script:
    - |
      ...
      # Final cleanup in after_script too
      killall -9 wine 2>/dev/null || true
      killall -9 wineserver 2>/dev/null || true
      wineserver -k 2>/dev/null || true
  artifacts:
...
And screenshot for output of notification stage that I don't include in this.
