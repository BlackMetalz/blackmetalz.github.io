...
Next we need to setup ssh key for clone without credential xD. Local dev = git clone via SSH. But pipeline needs PAT because self-hosted agent / ArgoCD need HTTPS auth!
...
Pipeline needs to auth into ACR to push image. The recommended way is Service connection using Service Principal.
...
- Subscription: pick subscription that contains ACR
