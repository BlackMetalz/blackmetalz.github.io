Title: Prepare for the Istio Certified Associate (ICA) Exam
Date: 2025-12-31
...
- **Control Plane Debugging:** Checking istiod logs, sync status, and xDS (x Discovery Service) configuration. 
...
| `MUTUAL` | mTLS - client cert required | High security, need correct keys: `tls.crt tls.key ca.crt` |
...
| Still like above but with namespace xD | `istioctl analyze -n default-namespace` | Configuration errors |
...
- Recommended: [ICA Certification | Killercoda](https://killercoda.com/ica-scenarios)
- [Istio | Killercoda](https://killercoda.com/istio)
- Recommended: [Killercoda Interactive Environments](https://killercoda.com/ica)
- Recommended, this covers 70% exam scope: [Istioworkshop.github.io](https://istioworkshop.github.io/)
- Free resource for learning: [Tetrate](https://academy.tetrate.io/courses/take/istio-fundamentals/lessons/19935842-1-0-module-overview)

### Practice Istio Installation
Scenario: Install Istio version 1.21.0 on a Kubernetes cluster using istioctl.
Requirements:

- Download Istio version 1.21.0
- Install Istio using `demo` profile
- Enable sidecar injection for `production` namespace
- Deploy a test application httpbin in `production` namespace
...
- Download Istio version `1.22.0`
- Install new control plane with revision `1-22-0` using `demo` profile
- Migrate `production` namespace to new revision
- Restart workloads and verify sync status
...
Document page for the exam: [https://istio.io/latest/docs/setup/upgrade/canary/](https://istio.io/latest/docs/setup/upgrade/canary/)